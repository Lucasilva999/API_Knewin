...
async function main() {
  ...
  axios.post('http://data.knewin.com/news', config)
    .then(async function(response){
        try {
          //Insere Notícia
          const quantidade_noticias = response.data.num_docs;
          const noticias = response.data.hits;
          for (let i = 0; i < noticias.length; i++) {

            const { url, content, title, source, published_date, source_id, id } = noticias[i];
            content ? content = content : content = 'Conteúdo_Não_Encontrado';
            const palavras = encontraPalavrasNoTexto(await definePalavarasQuery(), content.toLowerCase());
            const estado = noticias[i].source_locality[0].state;
            const uf = noticias[i].source_locality[0].stateAcronym;
          ...
          //Insere Log Notícia
          const ultimoCodigo = await LogNoticia.max('codigo');
          const ultimaPagina = await LogNoticia.max('pagina');
          const quantidade_gravada = noticias.length;
